 ...
 */
export function getWeekStartDate(weekIndex: number, month: number, year: number): string {
 ...
 */
export function getWeekIndexForDate(dateStr: string, month: number, year: number, availabilityData: any): number {
 ...
 * Updated to ensure consistent date alignment with calendar
 */
export function generateWeekData(weekIndex: number, month: number, year: number, availabilityData: any) {
  // Get the first day of the month
  const firstDay = new Date(year, month, 1)
  const firstDayOfWeek = firstDay.getDay() // 0 = Sunday
  ...
  const firstSunday = new Date(year, month, 1 - firstDayOfWeek)
 ...
 */
export function getTotalWeeksInMonth(month: number, year: number, availabilityData: any): number {
  const firstDay = new Date(year, month, 1)
...
export function findCurrentWeek(availabilityData: any) {
  ...
  const weekIndex = getWeekIndexForDate(todayStr, currentMonth, currentYear, availabilityData)
